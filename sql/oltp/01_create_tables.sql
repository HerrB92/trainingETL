...
-- CREATE SCHEMA IF NOT EXISTS is PostgreSQL syntax; T-SQL uses sys.schemas check
...
CREATE TABLE oltp.categories (
    ...
    parent_category_id INT           NULL REFERENCES oltp.categories(category_id)
);
...
CREATE TABLE oltp.suppliers (
    ...
    address_id    INT           NOT NULL REFERENCES oltp.addresses(address_id),
...
CREATE TABLE oltp.products (
    ...
    category_id INT             NULL REFERENCES oltp.categories(category_id),
    supplier_id INT             NOT NULL REFERENCES oltp.suppliers(supplier_id),
...
CREATE TABLE oltp.orders (
    ...
    status              NVARCHAR(30) NOT NULL DEFAULT 'PENDING' CHECK (status IN ('PENDING', 'CONFIRMED', 'SHIPPED', 'DELIVERED', 'CANCELLED')),
    shipping_address_id INT          NOT NULL REFERENCES oltp.addresses(address_id),
    created_at          DATETIME2    NOT NULL DEFAULT GETUTCDATE()
);
